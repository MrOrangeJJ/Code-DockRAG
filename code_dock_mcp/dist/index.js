...
// 参数验证函数
function isValidStandardSearchArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
        "query" in args &&
        typeof args.query === "string" &&
        (!("rerank" in args) ||
            typeof args.rerank === "boolean"));
}
function isValidStrongSearchArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
...
function isValidGetFilesArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
...
function isValidGetFileContentArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
...
function isValidBatchGetFileContentArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
        ...
        args.file_paths.every(path => typeof path === "string"));
}
function isValidFindReferencesArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
...
function isValidGetASTArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string");
}
function isValidSearchTextArgs(args) {
    return (typeof args === "object" &&
        args !== null &&
        "codebase_name" in args &&
        typeof args.codebase_name === "string" &&
...
// 检查代码库是否存在且已索引的辅助函数
async function checkCodebaseExists(codebase_name) {
    try {
        const response = await axios.get(`${API_BASE_URL}/codebases/indexed`);
        const codebases = response.data;
        return codebases.some(codebase => codebase.name === codebase_name &&
...
class CodeDockServer {
    constructor() {
        this.server = new Server({ name: "code-dock-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
        this.setupHandlers();
            ...
            switch (request.params.name) {
                ...
                case "standard_search": {
                    if (!isValidStandardSearchArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid standard_search arguments");
                    }
                    // 检查代码库是否存在且已索引
                ...
                case "strong_search": {
                    if (!isValidStrongSearchArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid strong_search arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
                ...
                case "get_files": {
                    if (!isValidGetFilesArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid get_files arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
                ...
                case "get_file_content": {
                    if (!isValidGetFileContentArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid get_file_content arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
                ...
                case "batch_get_file_content": {
                    if (!isValidBatchGetFileContentArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid batch_get_file_content arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
                ...
                case "find_references": {
                    if (!isValidFindReferencesArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid find_references arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
                ...
                case "get_ast": {
                    if (!isValidGetASTArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid get_ast arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
                ...
                case "search_text": {
                    if (!isValidSearchTextArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, "Invalid search_text arguments");
                    }
                    // 检查代码库是否存在且已索引
                    const codebaseExists = await checkCodebaseExists(request.params.arguments.codebase_name);
                    if (!codebaseExists) {
